...
```
    ...
    +----------+-----------+                   |
               ...
               v                               v
    +---------------------+       +------------------------+
    | Medical Review      |       |   RAG Knowledge Base   |
    | (AI necessity score,|       |   (HuggingFace local   |
    |  flags, assessment) |<------+    embeddings, 384-dim) |
    +----------+----------+       +------------------------+
               |                    policy_terms (11 chunks)
               v                    adjudication_rules (dynamic)
           AIContext                 medical_knowledge (8 chunks)
               |
               v
    +--------------------------------------------------+
    ...
    |                                                  |
    |  Falls back to deterministic pipeline on failure |
    +-------------------------+------------------------+
                              |
                              v
    +--------------------------------------------------+
    ...
    |                                                  |
    |  Decision summary, amount waterfall,             |
    |  line-item breakdown, counterfactuals,            |
    |  confidence breakdown, policy references          |
    +-------------------------+------------------------+
                              |
                +-------------+-------------+
                |                           |
